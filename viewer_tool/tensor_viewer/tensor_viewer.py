...
class TensorViewer(QWidget):
    ...
    def set_tensor(self, tensor):
        ...
        if type(self.tensor) == type(None):
            self.tensor = tensor
        elif str(list(self.tensor.shape)[1:]) == str(list(tensor.shape)[1:]):
            min = np.min([self.tensor.shape[0],tensor.shape[0]])
            self.tensor = self.tensor[:min,:,:]
            tensor = tensor[:min,:,:]

            self.tensor = np.concatenate([self.normalize(self.tensor), self.normalize(tensor)], axis=2)
            self.ui.slices.set_min(0)
            self.ui.slices.set_max(1)
        else:
