...
def preprocess(filepath, expected, output, debug = True, make_expected_mean_match = False, median=False, mask=None, soft_binarization=True):
    ...
    if ".tif" in filepath:#Do preprocessing all in one
        ...
        mean = None
        #difference = remove_frequency_from_pixel_vectors(difference, 10,20) no no no good
    ...
    expected = gaussian_filter_nan(expected,2)
    ...
    print(np.nanmax(difference))
    print(np.nanmin(difference))
    #difference -= np.min(difference)

    if soft_binarization:
        difference /= 50
        #difference = normalize(difference)
        difference = smooth_binarization(difference)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--soft_binarization', action="store_true")
    ...
    #Write some metainfo
    metainfo = {"source_folder": source_folder, "expected_images_folder": expected_images_folder}
    ...
    for name in files:
        ...
        preprocess(input, expected, out, args.debug, args.make_expected_mean_match, args.median, mask, args.soft_binarization)
