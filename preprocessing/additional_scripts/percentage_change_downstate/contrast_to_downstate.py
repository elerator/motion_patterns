...
def get_background(frames):
    ...
    difference = framewise_difference(frames.copy(), mean, bigdata=True)
    ...
    down_state = np.array(down_state, dtype=np.bool)
    
    mean = np.mean(frames[down_state], axis=0)
    return mean

def contrast_to_background(frames, mean, mask = None):
...
def percentage_change_downstate(frames, mask):
    res = contrast_to_background(frames, get_background(frames))
...
if __name__ == '__main__':
    ...
    for filenames in files:
        ...
        np.save(output, percentage_change_downstate(load_frames(inputs), mask))
