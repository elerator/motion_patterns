...
def process(input_file, id, target_folder, debug):
    dataset_path = os.path.join(target_folder,"dataset.pkl")
    if not os.path.isfile(dataset_path):
       dataset = defaultdict(dict)
       dataset["sws"] = defaultdict
       dataset["recordings"] = defaultdict
    else:
       print("Loaded datset")
...
if __name__ == '__main__':
    ...
    for input_file, output_file in zip(input_files, output_files):
        if not output_file:
           ...
           continue
        print(".", end="")
        process(input_file, os.path.basename(input_file).split(".")[0], target_folder, args.debug)
